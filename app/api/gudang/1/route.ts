...
export async function PATCH(request: NextRequest) {
  ...
  const updateDataGudang1 = await prisma.gudang1.update({
    where: { id_gudang },
    data: {
      id_distribusi,
      id_obat,
  ...
  return NextResponse.json(updateDataGudang1, {
    status: 200,
  });
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ message: 'Data Gudang 1 berhasil dihapus' }, { status: 200 });
  } catch (error) {
