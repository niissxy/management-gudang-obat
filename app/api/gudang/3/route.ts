...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const gudangLama = await prisma.gudang3.findUnique({
      where: { id_gudang },
    });

    if (!gudangLama) {
      return NextResponse.json({ error: "Data gudang 3 tidak ditemukan" }, { status: 404 });
    }
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      const gudangUpdate = await tx.gudang3.update({
        where: { id_gudang },
        data: {
          id_distribusi,
          id_obat,
    ...
    console.error("Gagal update gudang 3:", error);
    return NextResponse.json({ error: "Gagal update data gudang 3" }, { status: 500 });
  }
}
